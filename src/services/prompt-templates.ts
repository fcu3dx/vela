...
export const BUILTIN_PROMPTS: PromptTemplate[] = [
  ...
  5. **智能推荐结构与视角：如果作者已在配置中指定了故事结构（plotStructure）或叙事视角（narrativePOV），你必须严格使用作者指定的值，不要自行推荐或覆盖。只有在作者未指定的情况下，才根据类型推荐最合适的结构和视角。**`,
      systemSuffix: `【输出格式限制】
  ...
  【JSON 字段结构】
  {
      ...
      "plotStructure": "故事结构（three_act=三幕结构 / heros_journey=英雄之旅 / save_the_cat=节拍表 / kishotenketsu=起承转合 / multi_thread=多线叙事 / freeform=自由结构，**优先使用用户已指定的值**）",
      "narrativePOV": "叙事视角（third_limited=第三人称有限视角 / first_person=第一人称 / third_omniscient=第三人称全知视角 / multi_pov=多视角轮换，**优先使用用户已指定的值**）",
      "coreOutline": "核心大纲（不少于 150 字，含：主角的致命危机/开局困境、必须完成的核心目标、终极大危机、主要爽点起伏）",
      ...
      "writingStyle": "文风配置（不少于 100 字，涵盖：叙述节奏快慢与场景切换频率、描写密度偏好、对话风格与口语化程度、用词偏好古风/现代/专业术语、情感基调热血/冷峻/诙谐/沉重、标志性修辞手法与过渡技巧。请根据类型和受众推荐最匹配的写作风格）"
  }`,
    },
...
4. 每个角色至少150字详细展开（外貌、性格、背景、动机），图谱总字数不少于800字。
5. 仅返回角色图谱文本，不要任何客套话。
...
3. 每个维度至少展开200字，总字数不少于600字，确保世界观设定可以直接驱动情节。
4. 严格遵循全局写作要求与禁忌，禁止崩坏。
...
5. 大纲总字数不少于1000字，每个结构节点至少100字展开，确保写手拿到后能直接开工。
6. 绝不能触碰全局写作要求与禁忌中的毒点。
...
请直接输出精修后的全文章节内容。强制要求纯文本，禁止使用任何 Markdown 语法符号，严禁剧本式对话。【严禁】任何开场白或解释文字。**严禁输出"思考过程""推理步骤""分析""说明"等任何非正文内容。你的回答必须从第一个字就是正文，到最后一个字仍是正文。**
...
5. **必须输出完整的修复后全文，不是只输出修改部分——即使改动只有一个字，也要把整章从头到尾输出一遍**`,
    systemSuffix: `★【作者对本步骤的额外指导（如有，最高优先级）】★：
{{user_refine_prompt}}

请直接输出修复后的全文章节内容。强制要求纯文本，严禁剧本式格式，【严禁】任何开场白、解释文字。**严禁输出"思考过程""推理步骤""分析""说明"等任何非正文内容。你的回答必须从第一个字就是正文，到最后一个字仍是正文。**
